# language: pt
@regressivo
Funcionalidade: Excluir cadastro de medição de qualidade de água
  Como usuário da API
  Quero conseguir deletar um cadastro de medição de qualidade de água
  ...
  @padrão
  Cenário: Deve ser possível deletar um cadastro de medição de qualidade de água
    Dado que eu recupere o ID de medição de qualidade de água criado no contexto
    Quando eu enviar a requisição com o ID para o endpoint "/api/qualidade/agua" de deleção de medição de qualidade de água
    Então o status code da resposta da api de medição de qualidade de água deve ser 204

  @padrão
  Cenário: Deve falhar ao tentar deletar um cadastro de medição de qualidade de água que não existe
